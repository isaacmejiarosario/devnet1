...
""" Author: Nick Russo Purpose: Simple bootstrap ZTP script to enable remote access to a device using SSH. """ 

import cli

# Use "show version" to extract the device serial number, which is guaranteed to be unique. Capture the last string from the line which is always the 
# serial number. The output looks like this:
...
# Define serial to IP address 2-tuple matching, which provides the unique tunnel and loopback IPs for each device.

ipaddr_map = {
    "9RN0LZFPO0K": ("10.0.0.2", "172.16.100.2"),
...
#  1. Configure a unique hostname 2. Configure SSH with crypto keys 3. Create a basic username and VTY access methods 4. Configure loopback 5. Configure 
#  DMVPN spoke tunnel with OSPF/CDP
